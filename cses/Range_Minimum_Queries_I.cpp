...
class SegmentTree {
    public:
        ...
        shared_ptr<Node> build(const vector<ull>& v, ull from, ull to) {
            ...
            if (from == to)
                ...
                if (result->left != nullptr && result->right != nullptr)
                    result->val = min(result->left->val, result->right->val);
        ...
        ull query(shared_ptr<Node> tree, ull from, ull to) {
            ...
            if (tree->to < from || to < tree->from) return INT64_MAX;
            if (tree->left != nullptr && tree->right != nullptr)
                return min(query(tree->left, from, to), query(tree->right, from, to));
            else if (tree->left != nullptr)
                return query(tree->left, from, to);
            else if (tree->right != nullptr)
                return query(tree->right, from, to);
            return INT64_MAX;
        }
