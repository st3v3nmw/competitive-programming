...
fn main() {
    ...
    let g = sc.token::<u32>();
    for _ in 0..g {
        let (a, b, c, n) = (sc.token::<u32>(), sc.token::<u32>(), sc.token::<u32>(), sc.token::<u32>());
        let avg: f32 = (a + b + c + n) as f32 / 3.0;
        if a as f32 > avg || b as f32 > avg || c as f32 > avg {
            writeln!(out, "NO");
        } else {
            writeln!(out, "{}", ternary!(((a + b + c + n) as u32) % 3 == 0, "YES", "NO"));
        }
    }
}